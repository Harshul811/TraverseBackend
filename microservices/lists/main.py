...
@app.get("/weather/", response_model=float)
...
def get_lists(user_id: int, skip: int = 0, limit: int = 10, db: Session = Depends(get_db)):
    return crud.get_lists(db=db, user_id=user_id, skip=skip, limit=limit)
