...
def get_lists(db: Session, user_id:int, skip: int = 0, limit: int = 10):
    lists = (
        db.query(models.List)
        .filter(models.List.user_id == user_id)  # Filter by user_id
        .offset(skip)
        .limit(limit)
        .all()
    )
...
def fetch_weather(latitude: float, longitude: float, date: str, correlation_id: str) -> float:
    ...
    if response.status_code == 200:
        ...
        avg_temp_rounded = round(avg_temp, 2)  # Round to 2 decimal places
        return avg_temp_rounded
